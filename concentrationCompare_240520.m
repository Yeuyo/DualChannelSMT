...
%     plot(tracksE(ind, 2), tracksE(ind, 3), 'x');
    ...
    for m = 1 : length(c2Stack)
      ...
      intL = imquantize(intFil, multithresh(intFil, 20)); % 16+ is H, 13-15 M, 8+ in cell
...
%       plot(tracksE(ind, 2), tracksE(ind, 3), 'x');
    ...
    highInt(16)
    highInt(13)
    perctM = (numel(int(int>=13)) - numel(int(int>=16))) / length(int);
    perctL = (numel(int(int>=8)) - numel(int(int>=13))) / length(int);
