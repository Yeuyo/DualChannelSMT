...
c1Path = '\\shared.sydney.edu.au\research-data\PRJ-smFISHQ\Bitong\Concentration Compare\c1';
c1Path = '\\shared.sydney.edu.au\research-data\PRJ-smFISHQ\Bitong\Concentration Compare\Figure fot plot_20240820\c1';
% Folder containing mask files
c2Path = '\\shared.sydney.edu.au\research-data\PRJ-smFISHQ\Bitong\Concentration Compare\c20';
c2Path = '\\shared.sydney.edu.au\research-data\PRJ-smFISHQ\Bitong\Concentration Compare\Figure fot plot_20240820\c20';
% Plot figures?
toPlot = 0;
...
%   imshow(regionToDot)
  ...
  for m = 1 : length(col)
    ...
    soxNum = length(unique(tracksE(find(distSox < distTol), 5)))
    soxFrame = length(unique(tracksE(find(distSox < distTol), 4)))
    for o = 1 : nbImages
